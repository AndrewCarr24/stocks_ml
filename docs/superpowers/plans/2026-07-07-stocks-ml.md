...
- Mechanics (the no-lookahead core — implement exactly):
  ...
  2. At each rebalance `t`: refit `clone(estimator)` on `panel[(date <= t - purge_days) & label.notna()]` if it's the first fit or ≥ `retrain_weeks` weeks since the last fit; skip trading at `t` if that training set has < 200 rows.
