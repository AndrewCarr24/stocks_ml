# Feature screen: nested3_v1

Selection window 2006-01-01 -> 2015-12-31; label `label_4w`; probe weeks end inside the window (last rank date 2015-12-31 minus 35 days). Rule (registered, feature_screen.py): dense features keep iff |NW t| >= 2.0 (lag 4) and the same IC sign in both halves of the window; sparse flags (active < 10%) use the weekly mean label of flagged names. Keepers are examined as one bundle at the run's engine; PASS iff the top-6 paired calendar-HAC t > 2.0.
...
FAIL: top-6 paired HAC t +1.61 does not clear 2.0; nothing is admitted, the run proceeds on the standing features.
