...
def test_ledger_apply_second_call_is_idempotent_noop(tmp_path, monkeypatch, capsys):
    ...
    cmd_ledger(Namespace(action="init", file=None, force=False, cash=100.0, ledger="ledger.json", tag=None), cfg=None)
    capsys.readouterr()

    cmd_ledger(Namespace(action="apply", file=None, force=False, cash=100.0, ledger="ledger.json", tag=None), cfg=None)
